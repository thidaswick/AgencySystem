...
function SalesRepPaymentForm() {
  ...
  const handleSubmit = (e) => {
    ...
    const payments = JSON.parse(localStorage.getItem('salesRepPayments') || '[]');
    payments.push({ ...form });
    localStorage.setItem('salesRepPayments', JSON.stringify(payments));
    navigate('/sales-rep-payment-table');
  };
    ...
    <>
      ...
      <div className="container py-5 d-flex justify-content-center align-items-center" style={{ minHeight: '100vh' }}>
        <div className="card p-4 shadow" style={{ maxWidth: 500, width: '100%' }}>
          <h2 className="text-center mb-4 fw-bold">Sales Rep Payment</h2>
...
export default SalesRepPaymentForm; 